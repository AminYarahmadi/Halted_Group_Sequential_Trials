...
do_simulation <- function(sim=1, look_size, org_uppers, sprt_org_uppers, remaining_uppers, sprt_remaining_uppers, seed){
  ...
  stopping_look = interruption(Max_looks)#f(t)
  #print(stopping_look)
  observed_statistic = statistic_calculator(look_size, stopping_look-1, delta)
  #print(observed_statistic)#$observed_statistic
  org_boundary = org_uppers[1:stopping_look]
  sprt_org_boundary = sprt_org_uppers[1:stopping_look]
  #print(org_boundary)
  #print(sprt_org_boundary)
  rem_boundary = remaining_uppers[stopping_look]
  sprt_rem_boundary = sprt_remaining_uppers[stopping_look]
  #print(rem_boundary)
  #print(sprt_rem_boundary)
  
  stop_marker = 0
  
  for (j in seq(1, stopping_look)){
    if (stop_marker == 0){
      if(j < stopping_look){
        ...
        if(stopping_look == 1){
          CE_boundary = remaining_uppers[stopping_look]
        }
        else{
          CTIe = conditional_error(look_size, org_uppers, observed_statistic)#$CTI_err
          CE_boundary = CE_boundary(look_size, CTIe, observed_statistic)
        }
        for (i in 1:3){
          if (i == 1){
            if (((org_boundary[j] - observed_statistic[j]) > 0) == TRUE){
            ...
            if (((rem_boundary - observed_statistic[j]) > 0) == TRUE){
              rem_sim_result[j] = 1
            }
            else {
              rem_sim_result[j] = -1
  ...
  for (j in seq(1, stopping_look)){
    if (sprt_stop_marker == 0){
      if(j < stopping_look){
        ...
        if(stopping_look == 1){
          sprt_CE_boundary = sprt_remaining_uppers[stopping_look]
        }
        else{
          sprt_CTIe = sprt_CE(look_size, observed_statistic, stopping_look)#$CTI_err
          sprt_CE_boundary = CE_boundary(look_size, sprt_CTIe, observed_statistic)
        }
        for (i in 1:3){
          if (i == 1){
            if (((sprt_org_boundary[j] - observed_statistic[j]) > 0) == TRUE){
            ...
            if (((sprt_rem_boundary - observed_statistic[j]) > 0) == TRUE){
              sprt_rem_sim_result[j] = 1
            }
            else {
              sprt_rem_sim_result[j] = -1
...
interruption <-function(Max_looks){#f(t)
  #stopping_look = sample(1:Max_looks, 1)#round(runif(1,1,Max_looks))
  #stopping_look = sample(1:Max_looks, 1, prob=c(0.35, 0.25, 0.2, 0.1, 0.05, 0.03, 0.02, 0, 0, 0))#floor(rweibull(1, 3, 3)) + 1 #floor(rweibull(1, 2, 1)) + 1
  stopping_look = sample(1:Max_looks, 1, prob=c(0, 0, 0, 0.02, 0.03, 0.05, 0.1, 0.2, 0.25, 0.35))#floor(rbeta(1, 6, 3)*9 + 1)#floor(rbeta(1, 5, 2)*5 + 1)
  return(stopping_look)
}
#interruption(5)
#-------------------------------------------------------------------------------
...
conditional_error <- function (look_size, upper, observed_statistic){
  ...
  if (length(observed_statistic) == length(look_size)) {
    ...
    CTI_err[2] = 1-pmvnorm(upper = u, mean = J.mean[1:(n.looks-1)], sigma = J.sigma[1:(n.looks-1),1:(n.looks-1)])[1]
  }
...
CE_boundary <- function(look_size, CTI_err, observed_statistic){
  ...
  #print(J.sigma)
  #print(J.mean)
  new_upper = uniroot(search.function, target = CTI_err[2], mean = J.mean, sigma = J.sigma, interval=c(-100,100)*sqrt(look_size[counter+1]))$root
  #print(new_upper_new)
  return(new_upper) 
}
#-------------------------------------------------------------------------------
...
plot(xdata[order(xdata)], y1[order(xdata)], type = "o", pch = 1, col = "blue", lty=1, xlab = "Look", ylab = "Cumulative power", ylim=c(0,0.9))
